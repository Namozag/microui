 ...
 * @author h
 */
public class SelectionableGridCanvas extends GridCanvas {
    ...
    public SelectionableGridCanvas(GenericItem[] items, int itemWidth, int itemHeight) {
        super(items, itemWidth, itemHeight);
    ...
    protected void keyPressed(int keyCode) {
        ...
        // slide down
        if (items[selector.getPointer()].y + shift > screenHeight - itemHeight) {
            slideDown();
        }
//            shift -= itemHeight+margin;

        // slide up
        if (items[selector.getPointer()].y + shift < 0) {
            slideUp();
//            shift += itemHeight + margin;
        }
