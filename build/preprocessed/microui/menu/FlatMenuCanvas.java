 ...
 * @author h
 */
public class FlatMenuCanvas extends Canvas {

    Vector elementsLines = new Vector();
    ...
    public FlatMenuCanvas(NavigatableMenu menu) {
        this.menu = menu;
        if(menu.itemAt(0).image != null) {
        ...
        for (int i = 0; i < menu.size(); i++) {
            Vector l = TextManager.getParagraphLines(menu.itemAt(i).info, font, getWidth() - itemWS * 2);
            elementsLines.addElement(l);
        }
    }
    int selectorH = 0;

    private int getY(int i) {
        int s = i * (itemH + itemHS + font.getHeight() * 2) + shift;
...
//        g.drawImage(Main.background, 0, 0, Graphics.TOP|Graphics.LEFT);
...
//                g.drawRect(getWidth()-itemWS*5, y + itemH/3, itemH/3, itemH/3);
            ...
            g.setColor(STYLE.colorTextPointed);
            g.drawRect(itemWS - itemWS / 2 + 2, y - itemHS / 2 + 2, getWidth() - itemWS * 2 + itemWS - 4, itemH + itemHS + 2 * font.getHeight() - 4);
        }

        int i = menu.getPointer();
        int y = getY(i);
        g.setColor(0xFFFF00);
        g.fillRect(itemWS, y, itemWS, itemH);
//        g.fillRect(getWidth() - itemWS * 2, y, itemWS, itemH);
        g.setColor(STYLE.colorSelector);
        g.drawRect(itemWS - itemWS / 2, y - itemHS / 2, getWidth() - itemWS * 2 + itemWS, itemH + itemHS + 2 * font.getHeight());
        g.drawRect(itemWS - itemWS / 2 +1, y - itemHS / 2 +1, getWidth() - itemWS * 2 + itemWS -2, itemH + itemHS + 2 * font.getHeight() -2);
    ...
    private void paintInfo(Graphics g, int i) {
        ...
        g.setColor(STYLE.colorBG);
        g.fillRect(itemWS, y - itemHS, getWidth() - itemWS * 2, 2 * font.getHeight() + itemHS);
...
//        lines = TextManager.getParagraphLines(menu.itemAt(i).info, getWidth() - itemWS*2, font);
        lines = ((Vector) elementsLines.elementAt(i));
        for (int l = 0; l < lines.size(); l++) {
            g.drawString((String) lines.elementAt(l), getWidth() - itemWS * 2, y + l * font.getHeight(), Graphics.TOP | Graphics.RIGHT);
        }
    }
    ...
    public void keyPressed(int keyCode) {
        ...
        switch (key) {
            ...
            case LEFT:
                ...
                break;
            case FIRE:
                if (menu instanceof Selectionable && ENABLE_INFO) {
                    showInfo();
                }
                if(menu instanceof Selectionable)
